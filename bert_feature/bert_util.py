...
class Sst2Processor(object):
    ...
    def get_train_examples(self, data_dir, num_train_samples=-1):
        """See base class."""
        if num_train_samples != -1:
            return self._create_examples(self._read_tsv(os.path.join(data_dir, "emo_train.tsv")), "train")[: num_train_samples]
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "emo_train.tsv")), "train")

    def get_dev_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "emo_dev.tsv")), "dev")
...
def convert_examples_to_features(examples, label_list, max_seq_length, tokenizer):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        if example.label == '4':
            print("example.guid", example.guid)
            example.label = '0'
